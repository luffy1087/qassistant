...
function getXml(file, resolve, reject, promises, isDependency) {
    fs.readFile(file, function(err, result) {
        ...
        dependencies
            ...
            .forEach(function(projFile) { promises.push(new Promise(function(resolve, reject) { getXml.call(this, utils.strFormat('{0}\\{1}', assemblyProjPath, projFile), resolve, reject, promises, true); }.bind(this))); }.bind(this));

        utils.checkBeforeArrayConcatenating(csprojFiles, dependenciesProjFiles);

        if (isDependency) {
            console.log(assemblyName + ' is resolved');
        ...
        Promise.all(promises).then(function() {
            ...
            resolve({ proj: file, projPath: assemblyProjPath, xmlDoc: xmlDoc, assemblyName: assemblyName });
        });
    }.bind(this));
}
...
function onGettingFiles(env, dlls, resolve, files) {
    ...
    for (var i = 0; file = files[i]; i++) {
        promises.push(new Promise(function(resolve, reject) { getXml.call(this, file, resolve, reject); }.bind(this)));
    }
