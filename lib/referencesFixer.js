...
function onReceivingXmls(env, resolve, Xmls) {
    ...
    this.csprojDirs[env] = Xmls.map(function(json) { return path.dirname(json.proj) });
    resolve();
}

function getDependenciesByXml(xmlDoc) {
    var dependencies = xmlDoc.documentElement.getElementsByTagName('ProjectReference');

    if (dependencies.length === 0) { return dependencies; }

    return dependencies.map(function(dept) { return dept.getAttribute('Include')  });
}
...
function getXml(file, resolve, reject, promises, isDependency) {
    fs.readFile(file, function(err, result) {
        ...
        if (isDependency) { return void resolve({ proj: file, projPath: assemblyProjPath, xmlDoc: xmlDoc, assemblyName: assemblyName }); }
...
function tryChangeReference(mainSolutionReference, secondSolutionReference, mainSolutionCsprojPath, xmlDoc) {
    var mainReferenceJson = utils.referenceInfoToObject(mainSolutionReference.getAttribute('Include'));
    var secondReferenceJson = utils.referenceInfoToObject(secondSolutionReference.getAttribute('Include'));

    if (mainReferenceJson.Dll !== secondReferenceJson.Dll) {
        return; 
    }

    var mainReferencePath = mainSolutionReference.getElementsByTagName('HintPath')[0];
    var secondReferencePath = secondSolutionReference.getElementsByTagName('HintPath')[0];

    if (!mainReferencePath || !secondReferencePath) {
        return;
    }

    var textNode = xmlDoc.createTextNode(utils.strFormat('{0}\\{1}', mainSolutionCsprojPath, mainReferencePath.firstChild.nodeValue));
    //Remove TextNode Representing The Old Path
    secondReferencePath.removeChild(secondReferencePath.childNodes[0]);
    //Add A New TextNode Representing The New Path
    secondReferencePath.appendChild(textNode);
}

function tryChangeReferenceFromAssembly(assemblyName, assemblyPath, secondSolutionReference, xmlDoc) {
    var secondReferenceJson = utils.referenceInfoToObject(secondSolutionReference.getAttribute('Include'));
    ...
    if (!secondReferencePath) {
        return;
    }

    var textNode = xmlDoc.createTextNode(assemblyPath);
    //Remove TextNode Representing The Old Path
    secondReferencePath.removeChild(secondReferencePath.childNodes[0]);
    //Add A New TextNode Representing The New Path
    secondReferencePath.appendChild(textNode); 
...
function changeXmlCsproj(assemblyName, assemblyPath, mainSolutionReference, mainSolutionCsprojPath) {
    ...
    for (var csprojIndex = 0, csproj, references; csproj = solutionData[csprojIndex]; csprojIndex++) {
        ...
        for (var referenceIndex = 0, reference; reference = references[referenceIndex]; i++) {
            tryChangeReference(mainSolutionReference, reference, mainSolutionCsprojPath, csproj.xmlDoc);
