...
function getFileName(assemblyProjFileName, projFile) {
    return utils.strFormat('{0}\\{1}', utils.removeBackDirs(path.dirname(assemblyProjFileName), projFile));
}
...
function onReadXml(filePath, err, result) {
    ...
    this.addCsproj({ fileName: filePath, filePath: path.dirname(filePath), assemblyName: getAssemblyName(xmlDoc), xmlDoc: xmlDoc });
